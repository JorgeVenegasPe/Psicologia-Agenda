...
INSERT INTO `paciente` (`IdPaciente`, `NomPaciente`, `ApPaterno`, `ApMaterno`, `Dni`, `FechaNacimiento`, `Edad`, `GradoInstruccion`, `Ocupacion`, `EstadoCivil`, `Genero`, `Telefono`, `Email`, `Direccion`, `AntecedentesMedicos`, `IdPsicologo`, `MedicamentosPrescritos`, `FechaRegistro`, `CodigoPaciente`) VALUES
...
--
DELIMITER $$
CREATE TRIGGER `generar_codigo_paciente` BEFORE INSERT ON `paciente` FOR EACH ROW BEGIN
  DECLARE ultimo_codigo INT;
  SET ultimo_codigo = COALESCE((SELECT MAX(CAST(SUBSTRING(CodigoPaciente, 3) AS UNSIGNED)) FROM paciente), 0);

  IF ultimo_codigo < 9 THEN
    SET NEW.CodigoPaciente = CONCAT('PA00', ultimo_codigo + 1);
  ELSEIF ultimo_codigo >= 9 AND ultimo_codigo < 99 THEN
    SET NEW.CodigoPaciente = CONCAT('PA0', ultimo_codigo + 1);
  ELSE
    SET NEW.CodigoPaciente = CONCAT('PA', ultimo_codigo + 1);
  END IF;
END
$$
DELIMITER ;
...
(2, 'admin', 'admin', '2023-07-12 09:15:07', '');
