...
class UserModelCita
{
    ...
    {
        $statement = $this->PDO->prepare("SELECT COUNT(*) as total FROM cita WHERE IdPsicologo = :idPsicologo  AND CanalCita = :canal");
        $statement->bindParam(":idPsicologo", $id, PDO::PARAM_INT);
        $statement->bindParam(":canal", $canal, PDO::PARAM_STR);
        $result = $statement->execute();
